//! Demonstrates CLATPS, which solves a packed triangular system with overflow-protecting LAPACK scaling.
//! The matrix remains in standard lower-triangular packed-column storage; the
//! operation passes that packed slice directly to LAPACK without expanding it to a dense matrix.

use matrixpacked::{PackedLower, Diagonal, Transpose};
use num_complex::Complex32;

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let a = PackedLower::<Complex32>::from_vec(2, vec![Complex32::new(2.0, 0.0), Complex32::new(1.0, 0.0), Complex32::new(3.0, 0.0)])?;
    let mut b = [Complex32::new(2.0, 0.0), Complex32::new(7.0, 0.0)];
    let scale = a.solve_scaled_in_place(&mut b, Transpose::None, Diagonal::NonUnit)?;
    assert!(scale > 0.0);
    assert!((b[0] - Complex32::new(1.0, 0.0)).norm() < 1e-4);
    assert!((b[1] - Complex32::new(2.0, 0.0)).norm() < 1e-4);
    Ok(())
}
